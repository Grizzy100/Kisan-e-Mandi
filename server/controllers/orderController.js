...
export const adminUpdateOrderStatus = async (req, res) => {
  try {
    ...
    res.json({ message: `Order forcibly marked as ${status} by admin.`, order });
...
export const createOrder = async (req, res) => {
  try {
    ...
    if (item.minOrderQuantity && quantityNumber < item.minOrderQuantity) {
      return res.status(400).json({ message: `Minimum order quantity is ${item.minOrderQuantity} ${item.unit}.` });
    }
    ...
    // ── Respond immediately — vendor email fires in background with retry ───
...
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    // ── Respond immediately — buyer notification fires in background with retry ───
    res.json({ message: `Order marked as ${status}.`, order });

    const buyerDoc = await Order.findById(order._id).populate("customerId", "name email");
    const buyerEmail = buyerDoc.customerId?.email;
    if (buyerEmail) {
      fireEmail({
        to: buyerEmail,
        subject: `Order Update: Your order is now '${status}'`,
...
//This controller manages the complete order lifecycle—from order creation, 
// inventory reservation, and 
// state transition validation to order tracking, 
// cancellation, 
// email notifications, 
// admin management, and 
// sales analytics

// The admin acts as a platform moderator. Unlike buyers and sellers, 
// the admin can view all orders, 
// intervene in any order's lifecycle, 
// override ownership restrictions, and maintain an audit trail of every intervention
