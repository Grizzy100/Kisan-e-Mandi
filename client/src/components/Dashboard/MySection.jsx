...
export default function MySection() {
    ...
    const [postsSort, setPostsSort] = useState("newest"); // newest, oldest, likes, comments

    // Tickets State
    const [tickets, setTickets] = useState([]);
    const [ticketsLoading, setTicketsLoading] = useState(true);
    const [ticketsStatusFilter, setTicketsStatusFilter] = useState("All");
    const [ticketsSort, setTicketsSort] = useState("newest"); // newest, oldest
        ...
        <div className="max-w-4xl mx-auto py-6 px-4 space-y-8">
            ...
            <div className="flex flex-col gap-2">
                ...
                <p className="text-gray-500 text-sm">
                    Manage your posts and track your support tickets.
                </p>
            </div>

            {/* Animated Tabs */}
            <div className="flex p-1 bg-gray-100/80 rounded-2xl w-full sm:w-fit relative isolate">
                {["posts", "tickets"].map((tab) => {
                    const isActive = activeTab === tab;
                    return (
                        <button
                            key={tab}
                            onClick={() => setActiveTab(tab)}
                            className={`relative z-10 flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 text-sm font-medium transition-colors ${isActive ? "text-green-700" : "text-gray-500 hover:text-gray-700"
                                }`}
                        >
                            {tab === "posts" ? <MdArticle className="w-4 h-4" /> : <MdConfirmationNumber className="w-4 h-4" />}
                            <span className="capitalize">{tab === "posts" ? "Posts" : "Tickets Raised"}</span>
                            {isActive && (
                                <motion.div
            ...
            <div className="min-h-[500px]">
                {activeTab === "posts" ? (
                    <div className="space-y-6">
                        ...
                        {/* Posts List */}
                        {postsLoading ? (
                            <div className="text-center py-10"><div className="w-8 h-8 border-4 border-green-200 border-t-green-600 rounded-full animate-spin mx-auto" /></div>
                            ...
                            <div className="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0 scrollbar-hide w-full sm:w-auto">
                                {Object.keys(TICKET_STATUS_MAP).map((status) => (
                                    <button
                                        ...
                                        className={`px-4 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all ${ticketsStatusFilter === status
                                                ...
                                                <div className="px-6 py-5 space-y-4">
                                                    ...
                                                    {ticket.imageUrl && (
                                                        <div>
                                                            ...
                                                            <a href={ticket.imageUrl} target="_blank" rel="noopener noreferrer" className="block w-fit relative group">
                                                                <img src={ticket.imageUrl} alt="Attachment" className="max-h-64 rounded-xl border border-gray-200 shadow-sm" />
